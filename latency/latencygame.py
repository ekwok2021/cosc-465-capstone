...
def redrawGameWindow(score, LIVES, color_1, color_2, input_rect_1, input_rect_2, user_text_1, user_text_2, sub, timer_text, B, distance, bandwidth):
    ...
    q_text = "Given " + str(B) + "B packet, distance = " + str(distance) + "km, and bandwidth = " + str(bandwidth) + "Gbps, latency = ?ms"
...
def resultGameWindow(timeout, correct, range1, range2, answer):
...
def GAME():
    ...
    while level == GAME:
        ...
        if counter <= 0:
            timeout = True
            LIVES = LIVES - 1
            resultGameWindow(timeout, correct, range1, range2, answer)
            counter = MAXTIME
            timer_text = str(counter).rjust(3)
            sub = generateSubnet()
        ...
        # pygame.display.flip()
        redrawGameWindow(score, LIVES, color_1, color_2, input_rect_1, input_rect_2, user_text_1, user_text_2, sub, timer_text, B, distance, bandwidth)
